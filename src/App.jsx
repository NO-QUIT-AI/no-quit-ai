...
export default function App(){
  ...
  return (
    <div style={{minHeight:"100vh", background:"#0a0a0a", color:"#e5e5e5", fontFamily:"JetBrains Mono, monospace", display:"flex"}}>
      {/* Sidebar */}
      <div style={{width:160, background:"#080808", borderRight:"1px solid #1a1a1a", padding:"10px 8px", fontSize:11, display:"flex", flexDirection:"column", gap:8}}>
        <div style={{fontWeight:800, fontSize:10, letterSpacing:1}}>NO QUIT AI<br/><span style={{fontSize:7, color:"#666", fontWeight:400}}>ALWAYS BUILDING</span></div>
        <div style={{marginTop:12, color:"#666", fontSize:8}}>NAVIGATION</div>
        <div style={{background:"#fff", color:"#000", padding:"6px 8px", borderRadius:4, fontWeight:700}}>Dashboard</div>
        <div onClick={()=>setChat(true)} style={{padding:"6px 8px", color:"#888", cursor:"pointer"}}>New Chat</div>
        <div style={{marginTop:10, color:"#333", fontSize:8}}>RECENT CHATS</div>
        <div style={{color:"#555", fontSize:10}}>aslam o alaikum</div>
      </div>

      {/* Main */}
      <div style={{flex:1, padding:"12px 10px", maxWidth:600, margin:"0 auto"}}>
        {/* Header */}
        <div style={{display:"flex", gap:8, alignItems:"center", marginBottom:12}}>
          <span>⚡</span><div><div style={{fontWeight:800, fontSize:14}}>NO QUIT AI</div><div style={{fontSize:8, color:"#666"}}>Your personal AI coder - Builds instantly with live preview</div></div>
        </div>

        {/* Warning bar */}
        <div style={{background:"#121212", border:"1px solid #1e1e1e", borderRadius:6, padding:8, fontSize:8, marginBottom:10}}>
          <div style={{display:"flex", justifyContent:"space-between", color:"#666", letterSpacing:1}}>● ALL WORLD FUTURE WARNING - LIVE (Only ADD) <span>LIVE</span></div>
          <div style={{marginTop:6, background:"#000", padding:"6px 8px", borderRadius:4, display:"flex", justifyContent:"space-between", fontSize:9}}>
            <span>🌎 1 km NW of The Geysers, CA</span><span>M1.31 - 4:17:31 PM</span>
          </div>
        </div>

        {/* System Grow */}
        <div style={{background:"#111113", border:"1px solid #1e1e1e", borderRadius:8, padding:10, marginBottom:10}}>
          <div style={{display:"flex", justifyContent:"space-between", fontSize:8, color:"#22d3ee", letterSpacing:1}}>● SYSTEM GROW - AUTO IMPROVING <span style={{color:"#666"}}>LVL 81%</span></div>
          ...
          <div style={{fontSize:8, lineHeight:1.8, color:"#888"}}>
            <div>🚀 <span style={{color:"#22d3ee"}}>System upgraded: Build speed +12%</span></div>
            <div>🧠 Saving knowledge to memory...</div>
            <div>🔧 Self-improving prompt engine...</div>
            <div>💡 New component discovered: modern calculator</div>
            <div style={{color:"#555"}}>✓ Self-learning active | Not changing core system, only growing</div>
          </div>
        </div>

        {/* Auto Evolve */}
        <div style={{background:"#111113", border:"1px solid #1e1e1e", borderRadius:8, padding:10, marginBottom
cat > src/App.jsx << 'EOF'
import { useState } from 'react'
export default function App(){
  const [note,setNote]=useState(""); const [notes,setNotes]=useState([]); const [chat,setChat]=useState(false);
  return (
    <div style={{minHeight:"100vh", background:"#0a0a0a", color:"#e5e5e5", fontFamily:"monospace", display:"flex"}}>
      ...
      <div style={{flex:1, padding:"12px 10px"}}>
        ...
        <div style={{background:"#111113", border:"1px solid #1e1e1e", borderRadius:8, padding:10, marginBottom:10}}>
          <div style={{fontSize:8, color:"#a3a3a3"}}>● AUTO EVOLVE - WORLD AI COMPETITOR ANALYSIS</div>
        ...
        <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:8}}>
          <button onClick={()=>setChat(true)} style={{background:"#121212", border:"1px solid #1e1e1e", borderRadius:8, padding:14, textAlign:"left", color:"#fff"}}><div>🌐</div><div style={{fontWeight:700, fontSize:11, marginTop:6}}>Build Website</div><div style={{fontSize:8, color:"#666"}}>HTML & CSS site</div></button>
          <button onClick={()=>setChat(true)} style={{background:"#121212", border:"1px solid #1e1e1e", borderRadius:8, padding:14, textAlign:"left", color:"#fff"}}><div>{'</> '}</div><div style={{fontWeight:700, fontSize:11, marginTop:6}}>React App</div><div style={{fontSize:8, color:"#666"}}>Modern web app</div></button>
          <button onClick={()=>setChat(true)} style={{background:"#121212", border:"1px solid #1e1e1e", borderRadius:8, padding:14, textAlign:"left", color:"#fff"}}><div>>_</div><div style={{fontWeight:700, fontSize:11, marginTop:6}}>Python Script</div><div style={{fontSize:8, color:"#666"}}>Automation tool</div></button>
          <button onClick={()=>setChat(true)} style={{background:"#121212", border:"1px solid #1e1e1e", borderRadius:8, padding:14, textAlign:"left", color:"#fff"}}><div>🎮</div><div style={{fontWeight:700, fontSize:11, marginTop:6}}>Create Game</div><div style={{fontSize:8, color:"#666"}}>Canvas game</div></button>
        </div>
      </div>
